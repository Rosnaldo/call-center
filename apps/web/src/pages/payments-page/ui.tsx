...
const TOKEN_PACKS = [
  {
    ...
    description: 'Ideal para um atendimento rápido ou tirar dúvidas pontuais.'
  },
  {
    id: 'pack_standard',
    tokens: 15,
    ...
    description: 'Excelente custo-benefício. Garante cerca de 15 chamadas rápidas.'
  },
  {
    id: 'pack_premium',
    tokens: 40,
    ...
    description: 'Mais tempo de consulta sem interrupções por esgotamento.'
  }
];
  ...
  const handleBuy = async () => {
    ...
    try {
      if (!isSimulation) {
        ...
        if (!response.ok) {
          throw new Error('Falha ao registrar compra de tokens no servidor');
        }
      }
      ...
      console.error(err);
      alert('Houve um erro ao processar o pagamento no servidor. Tente novamente mais tarde.');
    } finally {
      ...
      <main className="flex-1 max-w-4xl w-full mx-auto px-4 py-8 sm:px-6 lg:px-8">
        ...
        {successInfo ? (
          ...
          <div className="bg-white border border-slate-200/80 rounded-2xl shadow-xl p-8 text-center max-w-xl mx-auto">
            ...
            <h1 className="text-2xl font-black text-slate-900 tracking-tight text-center">Compra Concluída com Sucesso!</h1>
            <p className="text-sm font-semibold text-emerald-600 mt-1 flex items-center justify-center gap-1.5 bg-emerald-50 max-w-fit mx-auto px-4 py-1.5 rounded-full select-none">
              ...
              +{successInfo.amount} tokens creditados na sua conta
            </p>

            <div className="my-8 py-5 border-y border-slate-100 text-left space-y-3.5">
              <div className="flex justify-between items-center text-xs">
                <span className="text-slate-400 font-medium">Situação de pagamento</span>
                <span className="text-emerald-600 font-semibold uppercase tracking-tight">Confirmado</span>
              </div>
              ...
              <div className="flex justify-between items-center">
                <span className="text-xs text-slate-500 font-semibold">Novo Saldo de Tokens:</span>
                <span className="text-sm font-black text-indigo-600 flex items-center gap-1 font-mono">
                  <Coins className="w-4 h-4" /> {currentTokensFromState} tokens
                </span>
              </div>
            </div>

            <p className="text-[11px] text-slate-400 leading-relaxed mb-6">
              Sua recarga foi integrada e creditada com segurança. Agora você tem tokens disponíveis para participar do serviço de atendimento.
            </p>
          ...
          <div className="space-y-8">
            ...
            <div className="bg-white border border-slate-200/80 rounded-2xl p-6 flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4">
              <div>
                <h1 className="text-xl font-bold text-slate-800 flex items-center gap-2">
                  ...
                  Comprar / Recarregar Tokens
                </h1>
                <p className="text-xs text-slate-400 mt-1">Recarregue tokens para garantir que suas chamadas continuem ativas na fila.</p>
              </div>
              <div className="bg-slate-100/70 border border-slate-200/50 px-4 py-3 rounded-xl flex items-center gap-3">
                ...
                <div>
                  <p className="text-[10px] uppercase font-bold tracking-widest text-slate-400">Seu Saldo</p>
                  <p className="text-base font-black text-slate-700 font-mono">{currentTokensFromState} tokens</p>
                </div>
              </div>
            </div>
            ...
            <div className="bg-white border border-slate-200/80 rounded-2xl p-6 sm:p-8 shadow-xs">
              ...
              <p className="text-xs font-bold text-slate-700 uppercase tracking-widest mb-6">
                Selecione um Pacote de Tokens ou insira o valor desejado
              </p>
              ...
              <div className="mb-8 p-4 bg-slate-50 border border-slate-200/50 rounded-xl">
                ...
                >
                  ...
                  <span>Desejo comprar uma Quantidade Customizada</span>
                </button>
              ...
              <div className="flex items-center gap-2.5 p-3.5 bg-indigo-50/40 rounded-xl border border-indigo-100/50 mb-8 text-xs text-indigo-900 select-none">
                ...
                <span>As ligações ativas gastam tokens periodicamente por minuto/segundos. Garantir fundos impede que sua chamada seja encerrada pelo sistema.</span>
              </div>
              ...
              >
                {isProcessing ? (
                  ...
                  <>
                    ...
                    Confirmar Recarga de Tokens (R$ {currentPrice.toFixed(2).replace('.', ',')})
                  </>
                )}
              </button>
            </div>
